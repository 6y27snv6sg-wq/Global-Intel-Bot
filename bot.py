...
from news_engine import (
    collect_news, search_news, build_ai_context,
)
...
# لكل مستخدم قفل مستقل؛ مستخدم لا يوقف بقية المستخدمين
    ...
    for i in range(0, len(items), 2):
        rows.append([
            InlineKeyboardButton(
                label, callback_data=f"topic:{key}"
            )
...
NEWS_TERMS = {
    ...
    "اليوم", "الآن", "الان", "حدث", "أحداث", "احداث",
    "الوضع", "تطورات", "مستجدات", "سياسة", "سياسي",
    "حرب", "هجوم", "ضربة", "قصف", "صراع", "أزمة", "ازمة",
    "روسيا", "أوكرانيا", "امريكا", "أمريكا", "الصين", "آسيا",
    "إيران", "ايران", "إسرائيل", "اسرائيل", "غزة",
    "السعودية", "الخليج", "الإمارات", "الامارات", "قطر",
    "الكويت", "البحرين", "عمان", "تركيا", "بريطانيا",
    "أوروبا", "اوروبا", "النفط", "الطاقة", "أوبك", "اوبك",
    "اقتصاد", "اقتصادية", "دولار", "أسواق", "اسواق",
    "أمن", "امن", "دفاع", "عسكري", "عسكرية",
    "مفاوضات", "اجتماع", "رئيس", "وزير", "وزارة",
    "تصريح", "بيان", "انتخابات", "برلمان",
    "news", "breaking", "latest", "world",
    "russia", "ukraine", "usa", "america", "china",
    "iran", "israel", "gulf", "oil", "energy",
}
...
def is_news_query(text: str) -> bool:
    ...
    # الأسئلة القصيرة جداً التي تحتوي اسم دولة/موضوع إخباري
    if any(term.lower() in t for term in NEWS_TERMS):
        return True

    # صيغ الأسئلة الإخبارية
    patterns = (
        r"\bما الجديد\b",
        r"\bماذا يحدث\b",
        r"\bماذا حدث\b",
...
async def get_fresh_news():
    try:
        return await asyncio.wait_for(
            asyncio.to_thread(collect_news, max_items=100),
...
"""
    ...
    # إذا تعذر Gemini، لا يتوقف البوت
    if result:
        return result

    lines = ["📰 أبرز الأخبار المرتبطة بسؤالك:\n"]
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with lock:
        ...
        try:
            ...
            if not results:
                await status.edit_text(
                    "🔎 لا توجد أخبار مرتبطة بهذا المجال حالياً."
                )
                return
...
async def handle_user_message(
    ...
    async with lock:
        ...
        try:
            ...
            report = await generate_report(text, results)

            await status.delete()
            await send_long_message(update, report)

        except Exception:
            log.exception("User message handler error")
            await status.edit_text(
                "⚠️ حدث خطأ مؤقت أثناء معالجة الطلب."
            )
...
def main():
    ...
    app.add_handler(
        CallbackQueryHandler(button_handler, pattern=r"^topic:")
    )
    app.add_handler(
        MessageHandler(
